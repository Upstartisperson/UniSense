...
namespace UniSense
{
	...
	public class OldDepreciatedDualSense : MonoBehaviour, IDualSenseManager
	{
		...
		{
			DS5W_ReturnValue status = (oS == OS_Type._x64) ? DS5W_x64.initDeviceContext(ref controller.devices.enumInfoBT, ref controller.devices.contextBT) :
		...
		private void DS5WenumDevices(ref DeviceEnumInfo[] infos, int Arraysize)
		{
			...
			DS5W_ReturnValue status = (oS == OS_Type._x64) ? DS5W_x64.enumDevices(ref ptrDeviceEnum, (uint)Arraysize, ref devicecount, false) :
		...
		public void SetCurrentGamepad(DualSenseController gamepad)
        {
            ...
            switch (senseController.connectionStatus)
            {
				...
				case DualSenseConnectionStatus.BT:
					...
					DS5W_ReturnValue status = (oS == OS_Type._x64) ? DS5W_x64.setDeviceRawOutputState(ref senseController.devices.contextBT, rawDeviceCommand, rawDeviceCommand.Length) :
